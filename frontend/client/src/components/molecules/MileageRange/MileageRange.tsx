...
import { formatMileageInput, handleUserInput } from '../../../lib/formatters';
import { Input } from '../../atoms';

interface MileageRangeProps extends BaseComponentProps {
  fromValue?: number;
  toValue?: number;
...
const MileageRange: React.FC<MileageRangeProps> = ({
  ...
  const formatInputValue = (value?: number) => {
    return formatMileageInput(value);
  };
  ...
  const handleFromInputChange = (newValue: string) => {
    ...
    const { numericValue } = handleUserInput(newValue, currentValue, 'км');
