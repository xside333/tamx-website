import { useFavoriteCars } from '../../../hooks';
...
const FavoritesPopover: React.FC<FavoritesPopoverProps> = ({
  ...
  // Загружаем данные автомобилей по их ID
  const { favoriteCars, isLoading } = useFavoriteCars(favoriteIds);
            ...
            <div className="space-y-3">
              {favoriteCars.map((car) => (
                <div
                  key={car.id}
                  className={cn(
                    "relative border border-muted rounded-lg p-3 transition-fast group",
                    !car.unavailable && "cursor-pointer hover:bg-surface-secondary"
                  )}
                  onClick={!car.unavailable ? () => handleCarClick(car.id) : undefined}
                >
                  {car.unavailable ? (
                    // Для проданных авто - только текст и кнопка удаления
                    <div className="flex items-center justify-between">
                      <span className="text-secondary text-sm">Автомобиль продан</span>
                      <button
                        onClick={(e) => handleRemoveClick(e, car.id)}
                        className="w-8 h-8 rounded-full bg-surface hover:bg-muted transition-fast flex items-center justify-center"
                      >
                        <Icon name="close" size="xs" className="text-secondary" />
                      </button>
                    </div>
                  ) : (
                    // Для доступных авто - полная информация
                    <div className="flex space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-secondary mb-2">
                          {car.year || '—'}, {typeof car.mileage === 'number' && car.mileage > 0 ? formatMileage(car.mileage) : '—'}, {getFuelLabel(car.fuel) || '—'}
                        </div>
                        
                        <div className="flex items-center justify-between">
                          <span className="text-accent font-bold text-sm">
                            {typeof car.price === 'number' && car.price > 0 ? formatPrice(car.price) : '—'}
                          </span>
